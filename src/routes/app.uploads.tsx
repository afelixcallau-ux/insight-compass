...
import { FileStack, Trash2, AlertTriangle, Loader2, Package, Users, FileText } from "lucide-react";
...
function UploadsPage() {
  ...
  const wipe = useMutation({
    ...
    onSuccess: () => {
      ...
      setConfirmOpen(null);
      toast.success("Datos borrados. La app está a cero.");
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl md:text-4xl font-semibold tracking-tight">Archivos subidos</h1>
        <p className="text-muted-foreground mt-1">Historial completo y gestión de datos</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Mis productos", value: counts?.mine ?? 0, icon: Package },
      ...
      <div className="glass-strong rounded-3xl p-5 border border-destructive/20">
        ...
        {confirmOpen && (
          <div className="mt-4 glass-subtle rounded-2xl p-4 flex items-center justify-between gap-3 flex-wrap">
            <p className="text-sm">
              ¿Confirmas borrar {confirmOpen === "all" ? "TODOS los datos (productos, competidores, archivos e insights)" : confirmOpen === "mine" ? "tus productos" : "todos los productos y competidores rivales"}?
            </p>
      ...
      <div className="glass rounded-2xl overflow-hidden">
        <div className="grid grid-cols-12 gap-3 px-4 py-3 text-xs text-muted-foreground border-b border-glass-border">
          ...
          <div className="col-span-2 text-right">Filas</div>
        ...
        {isLoading ? (
          ...
          <div key={u.id} className="grid grid-cols-12 gap-3 px-4 py-3 border-b border-glass-border text-sm items-center hover:bg-card/50 transition-colors">
            ...
            <div className="col-span-2 text-right">{u.rows_imported ?? 0}</div>
